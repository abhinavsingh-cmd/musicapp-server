...
// ---------------------------------------------------------------------------
// Freshness scenarios: prove the exact fallback priority chain
//   fresh YouTube → cached YouTube → library fallback → built-in fallback
// and that each failure mode preserves visible data.
// ---------------------------------------------------------------------------
describe('chartsStore — freshness scenarios', () => {
  it('fresh success: updates charts with LIVE source', async () => {
    mocks.getTrending.mockResolvedValue(makeResult({ source: 'LIVE' }));
    await useChartsStore.getState().fetchCharts();
  ...
  it('fresh failure with valid cache: preserves existing charts, shows error', async () => {
    // First fetch succeeds
    mocks.getTrending.mockResolvedValueOnce(makeResult({ source: 'LIVE' }));
    await useChartsStore.getState().fetchCharts();
    const before = useChartsStore.getState().topCharts;
    expect(before.length).toBe(3);

    // Second fetch (force) fails
    mocks.getTrending.mockRejectedValueOnce(new Error('Network error'));
    await useChartsStore.getState().fetchCharts({ force: true });

    const s = useChartsStore.getState();
    expect(s.topCharts).toEqual(before); // cache preserved
    expect(s.error).toBe('Network error');
    expect(s.source).toBe('LIVE'); // source unchanged
  });

  it('fresh failure without cache: shows error, empty state', async () => {
  ...
  it('empty response does not overwrite visible charts', async () => {
    // Seed with valid data
    mocks.getTrending.mockResolvedValueOnce(makeResult({ source: 'LIVE' }));
    await useChartsStore.getState().fetchCharts();
    const before = useChartsStore.getState().topCharts;

    // Force refresh returns empty
    mocks.getTrending.mockResolvedValueOnce({ songs: [], source: 'LIVE', lastUpdated: Date.now() });
    await useChartsStore.getState().fetchCharts({ force: true });

    const s = useChartsStore.getState();
    expect(s.topCharts).toEqual(before); // not wiped
    expect(s.error).toMatch(/previous data/i);
  });

  it('malformed response does not crash or overwrite charts', async () => {
    // Seed with valid data
    mocks.getTrending.mockResolvedValueOnce(makeResult({ source: 'LIVE' }));
    await useChartsStore.getState().fetchCharts();
    const before = useChartsStore.getState().topCharts;

    // Force refresh returns malformed data
    mocks.getTrending.mockResolvedValueOnce({ songs: null as any, source: 'LIVE', lastUpdated: Date.now() });
    await useChartsStore.getState().fetchCharts({ force: true });
  ...
  it('timeout does not crash or overwrite charts', async () => {
    // Seed with valid data
    mocks.getTrending.mockResolvedValueOnce(makeResult({ source: 'LIVE' }));
    await useChartsStore.getState().fetchCharts();
    const before = useChartsStore.getState().topCharts;

    // Force refresh times out
    mocks.getTrending.mockRejectedValueOnce(new Error('Request timed out'));
    await useChartsStore.getState().fetchCharts({ force: true });

    const s = useChartsStore.getState();
    expect(s.topCharts).toEqual(before); // not wiped
    expect(s.error).toBe('Request timed out');
  });

  it('refresh after cache: force:true always hits the network', async () => {
    mocks.getTrending.mockResolvedValue(makeResult({ source: 'LIVE' }));
    await useChartsStore.getState().fetchCharts();
    expect(mocks.getTrending).toHaveBeenCalledTimes(1);

    // Auto-fetch within fresh window: skips
    await useChartsStore.getState().fetchCharts();
    expect(mocks.getTrending).toHaveBeenCalledTimes(1);
  ...
  it('fallback-sourced data never blocks recovery to live', async () => {
    // Start with fallback data
    mocks.getTrending.mockResolvedValueOnce(makeResult({ source: 'BUILT_IN', origin: undefined }));
    await useChartsStore.getState().fetchCharts();
    expect(useChartsStore.getState().source).toBe('BUILT_IN');

    // Auto-fetch should NOT be blocked by the fresh window
    mocks.getTrending.mockResolvedValueOnce(makeResult({ source: 'LIVE', origin: 'youtube_music' }));
    await useChartsStore.getState().fetchCharts();
    expect(useChartsStore.getState().source).toBe('LIVE');
    expect(mocks.getTrending).toHaveBeenCalledTimes(2);
  });

  it('source labels are accurate: CACHED never contains "live"', async () => {
    mocks.getTrending.mockResolvedValue(makeResult({ source: 'CACHED', origin: 'youtube_music' }));
    await useChartsStore.getState().fetchCharts();

    const s = useChartsStore.getState();
    expect(s.source).toBe('CACHED');
    // The source is CACHED, not LIVE — the UI must not claim this is live
